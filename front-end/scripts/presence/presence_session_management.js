...
export async function showSessions(courseCode, courseName, isAdmin = false, page = 1, limit = 4) {
    ...
    sessionsGrid.innerHTML = `
        <div class="loading" style="text-align: center; padding: 2rem; color: #6B7280;">
    ...
    try {
        ...
        sessionsGrid.innerHTML = '';
        sessions.forEach(s => {
            const dateObj = s.date_heure ? new Date(s.date_heure) : new Date();
            ...
            cardDiv.addEventListener('click', () => showAbsences(courseCode, s.id_seance, s.titre || 'Séance', isAdmin));
            ...
            cardDiv.innerHTML = `
                <div class="seance-header">
                    ...
                    <div class="seance-info">
                        <div class="seance-title">${s.titre || 'Séance'}</div>
                        <div class="seance-course">${time}</div>
                    </div>
                    <span class="seance-status status-terminee">
        ...
        if (paginationContainer) {
            renderPagination(
                pagination,
                paginationContainer,
                (newPage) => showSessions(courseCode, courseName, isAdmin, newPage, limit) 
            );
        }
        ...
        sessionsGrid.innerHTML = `
            <p class="error" style="text-align: center; padding: 2rem; color: #EF4444;">
                Erreur lors du chargement des séances: ${error.message}
            </p>
        `;
    }
}

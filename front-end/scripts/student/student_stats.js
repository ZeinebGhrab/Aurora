import { getAllStudents } from "./student_api.js";


// Calcule les statistiques à partir de la pagination

export function calculateStatsFromPagination(pagination, students) {
    if (!pagination || !students) {
        return {
            total: 0,
            actifs: 0,
            nouveaux_semaine: 0,
    ...
    const actifsPage = students.filter(s => s.statut === 'activé').length;
    
    // Calculer les nouveaux cette semaine (7 derniers jours)
    const now = new Date();
    const oneWeekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    
    const nouveauxSemaine = students.filter(s => {
    ...
    const actifsEstimes = actifsPage > 0 ? 
        Math.round(total * (actifsPage / students.length)) : 
...
export function updateStudentStatsDisplay(stats) {
    if (!stats) {
...
export async function loadAndDisplayStats() {
    try {
        ...
        const { students, pagination } = await getAllStudents(1, 100, {});
        
        // Calculer les statistiques
        const stats = calculateStatsFromPagination(pagination, students);
        
        // Mettre à jour l'affichage
        updateStudentStatsDisplay(stats);

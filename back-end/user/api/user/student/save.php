...
$dossier = "uploads/";
if (!is_dir($dossier)) mkdir($dossier); // créer le dossier s'il n'existe pas
...
$stmt = mysqli_prepare($link, "INSERT INTO utilisateur (nom, prenom, email, mot_de_passe, photo_profil) VALUES (?, ?, ?, ?, ?)");
mysqli_stmt_bind_param($stmt, "sssss", $nom, $prenom, $email, $mot_de_passe_hash, $photo_profil);

if (mysqli_stmt_execute($stmt)) {
    $id_utilisateur = mysqli_insert_id($link); // récupérer l'id généré

    // Insérer dans etudiant avec la filière
    $stmt2 = mysqli_prepare($link, "INSERT INTO etudiant (id_etudiant, niveau, id_filiere) VALUES (?, ?, ?)");
    $niveau = '1ère année'; 
    mysqli_stmt_bind_param($stmt2, "isi", $id_utilisateur, $niveau, $filiere);
